...
import { InputConfig, Partecipant, PartecipantScores, SimulationResult } from '../src/types';
...
describe('Fair Lottery', () => {
  ...
  describe('with a given fair configuration', () => {
    ...
    it('should match the theoretical expectations', async () => {
      ...
      const expectedPartecipantWinRatio = _getTheoreticalPartecipantsWinRatio([...result.partecipantScores.keys()])

      for (const [partecipant, score] of result.partecipantScores.entries()) {
        const isBelowError = Math.abs(score/result.totalWin - expectedPartecipantWinRatio.get(partecipant)) < EXPECTED_ERROR
...
describe('Unfair Lottery', () => {
  ...
  describe('with a given unfair configuration', () => {
    ...
    it('favourite should sourpass the theoretical expectations', async () => {
      ...
      const isAboveError = result.partecipantScores.get(favouritePartecipant)/result.totalWin - expectedPartecipantWinRatio.get(favouritePartecipant) > EXPECTED_ERROR 
    ...
    it('unfavourites should fall below the theoretical expectations', async () => {
      ...
      for (const [partecipant, score] of result.partecipantScores.entries()) {
        if(partecipant != favouritePartecipant){
          const isBelowExpectations = score/result.totalWin - expectedPartecipantWinRatio.get(partecipant) < -EXPECTED_ERROR
